...
%if 0%{?rhel} == 8
%define python_binname python3.11
%define pip_binname pip3.11
%else
%define python_binname python3.11
%define pip_binname pip3.11
%endif
...
%if 0%{?rhel} == 8
%define install_crypto %{venv_python} %{venv_bin}/pip3.11 install cryptography==43.0.1
%else
%define install_crypto %{nil}
%endif

%define venv_pip %{venv_python} %{venv_bin}/pip3.11 install --find-links=%{wheel_dir} --no-index
...
%define pip_install_venv \
    ...
    %{install_venvctrl} \
    find %{venv_dir} -name pyvenv.cfg -exec sed -i "s|%{buildroot}||g" {} + \
    find %{venv_dir} -type f -name "*.dist-info" -exec sed -i "s|%{buildroot}||g" {} + \
